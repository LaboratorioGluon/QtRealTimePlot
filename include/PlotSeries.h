...
class PlotSeries : protected QOpenGLFunctions
{
   public:
    ...
    struct BlockStats {
        ...
        double max;
        double
            sum;  // Útil para calcular la media rápidamente (mean = sum / count)
        double
            sumSq;  // Útil para calcular el valor eficaz o desviación estándar (RMS)
        size_t count;
    };
    ...
    struct LodLevel {
        ...
        size_t                  lastProcessedCrudeSize =
            0;  // How many items have we already processed.
    };
