   ...
   public:
    ...
    void run() override
    {
        ...
        while (running)
        {
            ...
            for (int i = 0; i < 100; ++i)
            {
                double v1 = std::sin(2 * M_PI * 10.0 * t) +
                            0.3 * std::sin(2 * M_PI * 53.7 * t);
                double v2 = 5 * std::cos(2 * M_PI * 5.0 * t / 100) *
                            std::exp(-0.1 * std::fmod(t, 3.0));
                double v3 =
                    0.5 * std::sin(2 * M_PI * 1.0 * t) +
...
class MainWindow : public QMainWindow
{
   ...
   public:
    MainWindow()
    {
        ...
        ch2 =
            m_plot->addSeries("Ch2 - 5 Hz decaying", QColor(255, 180, 0), 5000);
        ch3 =
            m_plot->addSeries("Ch3 - 1 Hz + noise", QColor(80, 255, 120), 5000);
        m_plot->start();
        ...
        connect(
            cmbZoom, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            [this](int idx) {
        ...
        connect(btnGrid, &QPushButton::clicked, this, [this](bool) {
            this->m_plot->setGrid({!this->m_plot->getGrid().enabled,
                                   this->m_plot->getGrid().color});
        });
        ...
        connect(m_thread, &DataThread::newData, this,
                &MainWindow::newDataArrived);
    }
    ...
    void newDataArrived()
    {
        ...
        PlotSeries::IntervalStats stats =
            ch1->calculateIntervalStats(m_plot->getRangeCursor()->getPosStart(),
                                        m_plot->getRangeCursor()->getPosEnd());
        m_text->setPlainText(QString("Samples: %1").arg(stats.numSamples));
        m_text->appendPlainText(QString("Time: %1").arg(stats.timeDiff));
        m_text->appendPlainText(QString("Mean: %1").arg(stats.mean));
