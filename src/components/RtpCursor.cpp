...
RtpCursor::RtpCursor(int index, QColor color, MarkerStyle style) : m_color(color), m_cursorIndex(index), m_style(style), m_textColor(Qt::red), m_bgColor(Qt::black)
{
}

void RtpCursor::setFont(const QFont &font) { m_font = font; }
void RtpCursor::setTextColor(const QColor &color) { m_textColor = color; }
void RtpCursor::setBackgroundColor(const QColor &bgColor) { m_bgColor = bgColor; }
void RtpCursor::setMargin(const int marginPixels) { m_padding = marginPixels; }
...
bool RtpCursor::mousePressEvent(QMouseEvent *event, int currentPixelX)
{
    ...
    {
        m_isDragging = true;
        m_dragOffset = pos.x() - currentPixelX; // Usamos el píxel que el plot sabe que tenemos
        return true;
    }
    return false;
}

bool RtpCursor::mouseMoveEvent(QMouseEvent *event, const QRect &plotArea, int &outNewPixelX)
{
    if (!m_isDragging)
        return false;
...
void RtpCursor::drawFullMarker(QPainter &painter,
                               const QRect &plotArea,
                               int calculatedPixelX,
                               bool alignLeft)
{
    ...
    painter.drawText(QRect(textX, plotArea.top() + 5 + padding, alignLeft ? -boxWidth : boxWidth, lineHeight).normalized(), alignment, QString("M%1").arg(m_cursorIndex));
...
void RtpCursor::drawSimpleMarker(QPainter &painter,
                                 const QRect &plotArea,
                                 int calculatedPixelX,
                                 bool alignLeft)
{
...
void RtpCursor::draw(QPainter &painter,
                     const QRect &plotArea,
                     int calculatedPixelX,
